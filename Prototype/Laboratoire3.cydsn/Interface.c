...
/*void ActiverAlarme_task()
{
    for (;;)
    {
        if (VraiValeurBPMmax>BPMmax)
        {
            AlarmeOnBPMmax=true;
        }
        if (VraiValeurBPMmin<BPMmin)
        {
            AlarmeOnBPMmin=true;
        }
        if (VraiValeurSpO2>SpO2min)
        {
            AlarmeOnSpO2=true;
        }
...
void CreationVecteurEspacer(int longueurVecteur, int espacement, int ValeurMin, int ValeurMax)
{
    ...
    {
        VecteurEspacerIR[j]=(vectorIR[i]-ValeurMin)/(ValeurMax-ValeurMin);
        VecteurEspacerR[j]=(vectorR[i]-ValeurMin)/(ValeurMax-ValeurMin);
        j++;
    }
}
/*******************************************************************************
...
/*void CapSense_task()
{
    for (;;)
    {
        ...
        if(!CapSense_IsBusy())
        {
            ...
            //SÉLECTION DE PAGE
            if(CapSense_IsWidgetActive(CapSense_BUTTON0_WDGT_ID) && NumPage<=6 && NumPage!=1)//si qqun touche bouton0
                {
                    NumPage--;
                    ClearScreen();
            ...
            if(CapSense_IsWidgetActive(CapSense_BUTTON1_WDGT_ID) && NumPage>=1 && NumPage!=6)//si qqun touche bouton1
                {
                    NumPage++;
                    ClearScreen();
                    vTaskDelay(pdMS_TO_TICKS(50));
            ...
            if(CapSense_IsWidgetActive(CapSense_BUTTON0_WDGT_ID) && NumPage==1)//si qqun touche bouton0
                {
                    NumPage=6;
                    ClearScreen();
                    vTaskDelay(pdMS_TO_TICKS(50));
            ...
            if(CapSense_IsWidgetActive(CapSense_BUTTON1_WDGT_ID) && NumPage==6)//si qqun touche bouton1
                {
                    NumPage=1;
                    ClearScreen();
                    vTaskDelay(pdMS_TO_TICKS(50));
            ...
            //Bon code pour l'intensite des DELs
            if(pos!=0xFFFF && oldpos != pos && NumPage==3) //si le slider se fait toucher
            {
            ...
            //Bon code pour changer les valeurs min max, mais faut rajouter des conditions pour pas trop descendre ou trop monter
            if(pos!=0xFFFF && oldpos != pos && NumPage==4) //si le slider se fait toucher
            {
                SpO2min=SpO2min+(pos-50)/10;
                char message[10];
                sprintf(message, "%3d\r\n", SpO2min);
                GUI_DispStringAt(message, 150,107);
                
                oldpos=pos;
                vTaskDelay(pdMS_TO_TICKS(50));
                UpdateDisplay(CY_EINK_FULL_4STAGE, true);
            }
            if(pos!=0xFFFF && oldpos != pos && NumPage==5) //si le slider se fait toucher
            {
            ...
            if(pos!=0xFFFF && oldpos != pos && NumPage==6) //si le slider se fait toucher
            {
                BPMmax=BPMmax+(pos-50)/10;
                char message[10];
                sprintf(message, "%3d\r\n", BPMmax);
                GUI_DispStringAt(message, 160,107);
...
void Affichage_task()
{
    for (;;)
    {
        //if(OldNumPage!=NumPage || oldpos!=pos)
        //{
            
            if (NumPage==1 /*&& !CapSense_IsWidgetActive(CapSense_BUTTON1_WDGT_ID) && !CapSense_IsWidgetActive(CapSense_BUTTON0_WDGT_ID)*/)
            {
               ...
               /*if (AlarmeOnBPMmax)
                ...
                CreationVecteurEspacer(1000, 4, 94212, 104348);
            ...
            else if (NumPage==2 /*&& !CapSense_IsWidgetActive(CapSense_BUTTON1_WDGT_ID) && !CapSense_IsWidgetActive(CapSense_BUTTON0_WDGT_ID)*/)
            {
                ...
                CreationVecteurEspacer(1000, 4, 200993, 215520);
            ...
            else if (NumPage==3)
            {
                ...
                /*if (AlarmeOnBPMmax)
                {
                    ...
                    GUI_DispStringAt(bufferTRP, 120, 110);
                }
                else if (AlarmeOnBPMmin)
                {
                    GUI_SetColor(GUI_BLACK);
            ...
            else if (NumPage==6)
            {
                ...
                /*if (AlarmeOnBPMmax)
                {
                    GUI_SetColor(GUI_BLACK);
                    GUI_SetFont(GUI_FONT_16B_1);
                    char bufferT[40] = "ATTENTION: BPM trop haut";
                    GUI_DispStringAt(bufferT, 60, 6);
                ...
                else if (AlarmeOnBPMmin)
                {
                    GUI_SetColor(GUI_BLACK);
                    GUI_SetFont(GUI_FONT_16B_1);
                    char bufferT[40] = "ATTENTION: BPM trop bas";
                    GUI_DispStringAt(bufferT, 60, 6);
                ...
                else if (AlarmeOnSpO2)
                {
                    GUI_SetColor(GUI_BLACK);
                    GUI_SetFont(GUI_FONT_16B_1);
                    char bufferT[40] = "ATTENTION: SpO2 trop bas";
                    GUI_DispStringAt(bufferT, 60, 6);
                    */
                    char bufferTR[100] = "Changer la Valeur max du BPM:";
                    char bufferB[20] = "BPM Max : ";
