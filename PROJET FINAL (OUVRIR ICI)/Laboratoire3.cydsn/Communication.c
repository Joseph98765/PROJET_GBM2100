...
void MAX30102Config()
{
    ...
    MAX30102Write(REG_FIFO_CONFIG,
        (SMP_AVE_8 << SMP_AVE_RM_BIT) | (FIFO_ROLLOVER_EN_0 << FIFO_A_FULL_RM_BIT)
...
void MAX30102Task(void *arg){
    ...
    for(;;)
    {
        for(uint16_t i = 0; i < BUFFER_SIZE; i++){
            ...
            MAX30102ReadFIFO32bits(&buffer_Red_uint[i], &buffer_IR_uint[i]);
            buffer_Red[i] = (float) buffer_Red_uint[i];
            buffer_IR[i] = (float) buffer_IR_uint[i];
            printf("i: %u RED : %u  IR  : %u  \r\n",i,(uint32_t) buffer_Red_uint[i], (uint32_t) buffer_IR_uint[i]);
            if (i == BUFFER_SIZE/2){
                indexLecture = 0;
                flagTraitement = true;
