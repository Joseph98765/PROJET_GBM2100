...
void FilterLP(uint32_t* buffer, float* filteredSignal){
    ...
    {    
      arm_fir_f32(&S, (float32_t *)buffer + (i * blockSize) + indexCommunication, filteredSignal + (i * blockSize), blockSize);  
    } 
}
...
void FilterHP(float* buffer, float* filteredSignal){
    ...
    for(i=0; i < numBlocks; i++)  
    {    
      arm_fir_f32(&S, buffer + (i * blockSize) + indexCommunication, filteredSignal + (i * blockSize), blockSize);  
    } 
}
...
uint16_t CalculBPM(float* signalRedAC) {
    ...
    for(uint16_t i=windowSize; i < dataLength-windowSize; i++){
        if(signalRedAC[i-windowSize]<signalRedAC[i] && signalRedAC[i]<signalRedAC[i+windowSize] && signalRedAC[i]>1000){
            nPeak++;
            if (nPeak == 1){
                indexFirstPeak = i;
            }            
